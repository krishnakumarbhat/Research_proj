...
def _discover_project_files() -> list[Path]:
    return sorted(path for path in ROOT.glob("project_*.py") if path.is_file())
...
def _summarize_stderr(stderr: str, module_name: str) -> str:
    ...
    if not cleaned:
        return module_name
    return f"{module_name}; emitted {len(cleaned)} stderr lines"


def _run_file(path: Path, timeout: int = 240) -> ProjectFileRun:
    start = perf_counter()
    module_name = f"rlbench.{path.stem}"
    try:
...
def _write_markdown(results: list[ProjectFileRun], output_path: Path) -> Path:
    ...
    for result in results:
        safe_summary = (result.summary_line or "n/a").replace("|", "/")
        safe_details = (result.details or "").replace("|", "/")
        rows.append(
            f"| {result.file_name} | {result.status} | {result.runtime_seconds:.2f} | {safe_summary} | {safe_details} |"
        )
